...
export default function HelpCenterPage() {
    ...
    },
  {
    ...
    faqs: [
      ...
      {
        question: "What post types are available? Can I post as a room seeker?",
        answer: [
      ...
      <section className="relative py-12 md:py-16 px-4 bg-linear-to-r from-primary/10 to-accent/10">
        <div className="container mx-auto max-w-6xl text-center">
          <div className="flex items-center justify-center gap-4 mb-6">
            ...
            <h1 className="text-3xl md:text-5xl font-bold text-primary">Roomezy Help Center</h1>
          </div>
          <p className="text-lg md:text-xl text-muted-foreground max-w-4xl mx-auto leading-relaxed px-4">
            Your go-to resource for navigating Roomezy. Find quick answers, learn tips, and get support for a seamless shared living experience. Explore detailed FAQs, manage your posts, and connect with our community.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-6xl">
                ...
                {faqCategories.map((category) => {
                  ...
                  return (
                    <Button
                      ...
                      className={`flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 text-sm md:text-base ${activeCategory === category.id ? 'bg-primary text-primary-foreground' : 'hover:bg-primary/10'}`}
                    >
              ...
              <Card className="bg-card border border-border shadow-lg">
                <CardHeader className="bg-muted/20 p-4 md:p-6">
                  <CardTitle className="flex items-center gap-3 text-xl md:text-2xl">
                    <div className={`p-2 md:p-3 rounded-lg ${currentCategory.color} text-white`}>
                      <currentCategory.icon className="h-5 w-5 md:h-6 md:w-6" />
                    </div>
                    {currentCategory.title}
                    <Badge variant="secondary" className="ml-auto text-sm">{currentCategory.faqs.length} FAQs</Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-4 md:p-6">
          ...
          {searchTerm && (
            <div className="mt-6 md:mt-8">
              <h2 className="text-xl md:text-2xl font-bold mb-4 text-primary">Search Results</h2>
              <div className="space-y-4">
                    ...
                    <CardContent className="p-4 md:p-6">
                      {category.faqs.map((faq, idx) => (
                        <div key={idx} className="mb-4 last:mb-0">
                          <h4 className="font-semibold mb-2 text-base md:text-lg">{faq.question}</h4>
                          <ul className="space-y-1">
                            {faq.answer.map((point, pidx) => (
                              <li key={pidx} className="flex items-start gap-2 text-sm md:text-base text-muted-foreground">
