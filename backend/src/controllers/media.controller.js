...
export const createMediaPost = asyncHandler(async (req, res) => {
    ...
    if (files.video && files.images) {
        ...
        throw new ApiError(
            400,
            "You can upload either a video or images, not both."
        );
    }
    ...
    if (!files.video && !files.images) {
        throw new ApiError(
            400,
            "You must upload at least one image or one video."
        );
    }
    ...
    return res
        ...
        .json(
            new ApiResponse(
                201,
                populatedMedia,
                "Media post created successfully"
            )
        );
});
...
export const toggleLikeMedia = asyncHandler(async (req, res) => {
    ...
    // Get updated post with proper population and isLiked status
    ...
    // Update the media document for consistency
